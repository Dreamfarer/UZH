...
/* First attempt of buildMaxHeap, without heapify (works)

void buildMaxHeap(int A[], int n, int d){
    
    for (int i=(n-2)/d;i>=0;i--){ //c=(n-1) is the index of last child, (c-1)/d is the parent index of any c, so (n-2)/d is the largest parent index for heap of size n

        //i is parent index with children, starts at the largest
        int m=i; 
        int nextnode = i*d+1; //index of first child
        
        for (int j=0;j<d;j++){ //go through all d children of current node
            if(A[m]<A[nextnode] && nextnode < n){//if larger elem is found in children and we are in range of heap

                m = nextnode; //remember the index of larger elem
            } 
            nextnode++; //increment nextnode to get next child
        }

        if (i != m){
            //swap if a larger child is found
            int tempo = A[m];
            A[m] = A[i];
            A[i]=tempo;
